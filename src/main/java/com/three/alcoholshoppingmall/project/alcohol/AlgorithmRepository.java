...
public interface AlgorithmRepository extends JpaRepository<Alcohol, Long> {
    ...
    @Query(value = "SELECT REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(a.finish, ',', numbers.n), ',', -1), '\"', ''), ' ', '') AS word " +
            "FROM alcohol a INNER JOIN purchase p ON a.name = p.name JOIN (SELECT 1 + units.i + tens.i * 10 AS n FROM " +
            ...
            "WHERE p.email = :email GROUP BY word ORDER BY SUM(p.amount) DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(a.finish, ',', numbers.n), ',', -1), '\"', ''), ' ', '') AS word " +
            ...
            "WHERE p.email = :email GROUP BY word " +
